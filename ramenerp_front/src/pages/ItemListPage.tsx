...
const NOTUSED_ROUTE = "/items/notused";
...
function DaysCell({ d, warn }: { d: number | null | undefined; warn: number }) {
  ...
  return (
    <span style={{ color: ui.warn, fontWeight: 700 }}>
      D-{d}
    </span>
  );
}
...
const ItemListPage: React.FC<ItemListPageProps> = ({ hide_title = false }) => {
  ...
  // 편집 중인 행의 DB PK를 string으로 저장 (row.id 기반)
  const [editing_pk, set_editing_pk] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (exp_f && Number(exp_f) > 0) {
          params.set("expiring", String(Number(exp_f)));
        }
        ...
        const hydrated: ProductRow[] = normalized
          .map((it: ProductRow) => ({
            ...it,
            category_name:
              it.category_name ??
              (it.category_id ? cat_map.get(it.category_id) : undefined),
            vendor_name:
              it.vendor_name ??
              (it.vendor_id ? ven_map.get(it.vendor_id) : undefined),
            expiry_date: yyyy_mm_dd(it.expiry_date),
          }))
          .map((it: ProductRow) => ({
            ...it,
            days_left: days_left(it.expiry_date),
          }));
  ...
  const visible = useMemo(() => {
    ...
    const selected_vendor_name = ven_f ? ven_name_by_id.get(ven_f) ?? "" : "";

    return rows.filter((r: ProductRow) => {
      if (ven_f) {
        const row_vendor_name =
          r.vendor_name ?? (r.vendor_id ? ven_name_by_id.get(r.vendor_id) : "");
        const id_matches = r.vendor_id === ven_f;
        const name_matches = selected_vendor_name && row_vendor_name === selected_vendor_name;
        if (!id_matches && !name_matches) return false;
      }
  ...
  const start_edit = (r: ProductRow) => {
    ...
    // 여기서 중요한 건 row.id (DB PK)로 편집상태를 추적하는 것
    set_editing_pk(r.id != null ? String(r.id) : null);
  ...
  const save_edit = async () => {
    if (!editing_pk) return;

    const orig = rows.find((r) => String(r.id ?? "") === editing_pk);
    if (!orig) {
      alert("원본 데이터를 찾을 수 없습니다.");
      return;
    }

    const final_name = String((draft.name ?? orig.name ?? "").trim());

    const final_category_id = draft.category_id
      ? Number(draft.category_id)
      : Number(orig.category_id);

    const final_unit_id = draft.unit_id ? Number(draft.unit_id) : Number(orig.unit_id);

    const final_unit_price = (() => {
      const cand =
        typeof draft.unit_price === "number"
          ? draft.unit_price
          : Number(draft.unit_price ?? orig.unit_price ?? 0);
      return Number.isFinite(cand) ? cand : 0;
    })();

    const final_expiry_date = String(
      (draft.expiry_date ?? orig.expiry_date ?? "").trim()
    );

    const final_vendor_id = draft.vendor_id
      ? Number(draft.vendor_id)
      : (orig.vendor_id ? Number(orig.vendor_id) : undefined);
    ...
    for (const [k, v] of Object.entries(draft_payload)) {
      if (v !== undefined && v !== null && v !== "") {
        payload[k] = v;
      }
    }
    if (!("expiry_date" in payload)) {
      if (orig.expiry_date && orig.expiry_date.trim() !== "") {
        payload["expiry_date"] = orig.expiry_date;
      }
    }
    ...
    try {
      const res = await fetch(
        `/api/items/${encodeURIComponent(editing_pk)}`,
        {
          method: "PUT",
          headers: {
  ...
  const mark_notused = async (r: ProductRow) => {
    ...
    try {
      // change_item_use_state는 /api/items/:id/state 가 아니라
      // 백엔드에선 PUT /items/changestate/:id 이지만
      // 프론트 유틸이 이미 /api/items/:id/state 로 되어있다면
      // 그 부분은 api/items.ts 쪽을 백엔드에 맞춰 조정해줘야 한다.
      // 여기서는 그대로 호출한다고 가정.
      await change_item_use_state(Number(r.id), "NOTUSED");

      // 완료 후 미사용 품목 페이지로 이동
      navigate(NOTUSED_ROUTE);
    } catch (e: any) {
      ...
      <div style={bar}>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <input
            ...
            placeholder="예: 7"
            value={exp_f}
            onChange={(e) => set_exp_f(e.target.value)}
            style={ipt}
          />
        </label>
        ...
        <table style={table}>
          ...
          <tbody>
            {visible.map((r: ProductRow, idx: number) => {
              const is_edit = editing_pk === String(r.id ?? "");
              ...
              if (!is_edit) {
                return (
                  <tr key={r.id ?? r.item_id} style={bg}>
                    ...
                    <td style={td}>
                      <DaysCell
                        d={r.days_left ?? days_left(r.expiry_date)}
                        warn={warn_threshold}
                      />
                    </td>
                    <td style={td}>
              ...
              return (
                <tr key={r.id ?? r.item_id} style={bg}>
                  ...
                  <td style={td}>
                    <DaysCell d={draft_days} warn={warn_threshold} />
                  </td>
