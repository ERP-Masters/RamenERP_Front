...
type VendorOpt = { id: string; name: string }; // vendor_summary 대신 master_data 기반으로 변환한 형태
...
const ItemListPage: React.FC<ItemListPageProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // 거래처 옵션 가공
        // fetch_vendors() -> VendorOption[] { id: number; name: string }
        // 화면에서는 id를 string으로 들고 다니므로 변환
  ...
  }, [cat_f, exp_f, reload]); // ven_f는 프론트 필터만 사용
  ...
  const visible = useMemo(() => {
    ...
    return rows.filter((r: ProductRow) => {
      ...
      if (ven_f) {
        ...
        const nameMatches =
          selectedVendorName && rowVendorName === selectedVendorName;
  ...
  const save_edit = async () => {
    ...
    try {
      const res = await fetch(
        `/api/items/${encodeURIComponent(editing_id)}`,
        {
          method: "PUT",
          headers: {
