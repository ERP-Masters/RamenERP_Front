...
function normalize_item(raw: any): ProductRow {
  ...
  const cat_id =
    raw.category_id ?? raw.category?.id ?? raw.category?.category_id ?? raw.categoryId;
...
const ItemListPage: React.FC<ItemListPageProps> = ({ hide_title = false }) => {
  ...
  const [id_f, set_id_f] = useState<string>("");   // 🔎 ID 기반 필터
  const [name_f, set_name_f] = useState<string>(""); // 🔎 이름 기반 필터
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // 쿼리 파라미터 (카테고리/이름 검색만 서버로 보냄)
        const params = new URLSearchParams();
        if (cat_f) params.set("category", cat_f);
        if (name_f.trim()) params.set("search", name_f.trim()); // 서버 검색 활용
        ...
        if (!listRes.ok) {
          throw new Error(`HTTP ${listRes.status}: ${await listRes.text().catch(() => "")}`);
        }
  ...
  const visible = useMemo(() => {
    ...
    return rows.filter((r: ProductRow) => {
      // 거래처 필터
      if (ven_f) {
        const rowVendorName = r.vendor_name ?? (r.vendor_id ? ven_name_by_id.get(r.vendor_id) : "");
        const idMatches = r.vendor_id === ven_f;
        const nameMatches = rowVendorName && rowVendorName === (ven_name_by_id.get(ven_f) ?? "");
      ...
      // 이름 부분일치 (서버 검색을 쓰더라도 클라이언트에서 한 번 더 안전 필터)
      if (nameq) {
  ...
  const save_edit = async () => {
    ...
    const payload: Record<string, unknown> = {
      ...
      // 👇 유통기한 컬럼/필드 제거에 따라 payload에서 제외
      // expiry_date: ...
    };
    ...
    <div style={page}>
      ...
      <div style={bar}>
        ...
        <button
          ...
          style={{ ...ghost, background: ui.p_bg, color: ui.p_tx, borderColor: ui.p_bd }}
        >
          신규 품목 등록
        </button>
      </div>
      ...
      {error && (
        <div style={{ color: ui.danger, marginBottom: 8 }}>
          {error}
        </div>
      )}
              ...
              if (!is_edit) {
                return (
                  <tr key={r.id ?? r.item_id} style={bg}>
                    ...
                    <td style={td}>
                      {r.category_name ?? (r.category_id ? cat_name_by_id.get(r.category_id) : "")}
                    </td>
                    <td style={td}>
                      {(r.unit_id && unit_code_by_id.get(r.unit_id)) ?? r.unit_name ?? r.unit_id}
                    </td>
                    ...
                    <td style={td}>
                      <button onClick={() => start_edit(r)} disabled={Boolean(acting_id)} style={{ ...act, marginRight: 6 }}>
                        수정
                      </button>
                      <button
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              ...
              <button type="button" onClick={() => set_open(false)} style={ghost} aria-label="close">
                ×
              </button>
            </div>
