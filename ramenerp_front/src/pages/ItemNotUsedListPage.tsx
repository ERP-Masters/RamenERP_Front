...
const toArray = (j: any): any[] =>
  Array.isArray(j) ? j : j?.items ?? j?.data ?? [];
...
const ItemNotUsedListPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // 1) 미사용품목 목록 불러오기
        // 실제 백엔드에 맞게 조정 필요:
        //   - /items/state?isused=NOTUSED 가 맞으면 그대로 쓰면 되고
        //   - 아니라면 /api/items/notused 등으로 바꾸면 돼.
        const listJson =
          (await safeJson(
            "/items/state?isused=NOTUSED",
          )) ??
          (await safeJson(
            "/api/items/state?isused=NOTUSED",
          ));
        if (listJson === null)
          throw new Error("미사용 품목 목록 호출 실패");

        const src = toArray(listJson);

        // 2) 마스터: 카테고리 / 단위 / 거래처
        const [catsJ, unitsJ, vendorsRaw] = await Promise.all([
          (safeJson("/api/category") ??
            safeJson("/category")),
          (safeJson("/api/units") ??
            safeJson("/units")),
        ...
        // 화면은 string id로 다루므로 변환
        const vendors: VendorOpt[] = (vendorsRaw ||
          []
        ).map((v: VendorOption) => ({
          id: String(v.id),
        ...
        // map 준비
        const venMap = new Map<string, string>(
          vendors.map((v) => [v.id, v.name]),
        );

        // 3) 목록 표준화 + 거래처명 매핑
        const normalized: Row[] = src.map(
          (r: any): Row => {
            const vendor_id_raw =
              r.vendor_id !== undefined
                ? r.vendor_id
                : r.vendor?.id ??
                  r.vendor?.vendor_id;
            const vendor_id =
              vendor_id_raw !== undefined
                ? norm(vendor_id_raw)
                : undefined;
  ...
  const onUse = async (r: Row) => {
    ...
    if (
      !window.confirm(
        `'${r.item_id} - ${r.name}'을(를) 사용 전환할까요?`,
      )
    )
      return;

    try {
      await change_item_use_state(
        Number(r.id),
        "USED",
      );
      setRows((prev: Row[]) =>
