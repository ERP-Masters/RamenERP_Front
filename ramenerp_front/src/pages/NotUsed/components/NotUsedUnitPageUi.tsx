// src/pages/NotUsedUnitPageUi.tsx
import React, { useEffect, useState } from "react";
import { fetchNotUsedUnits, type ApiUnit } from "../function/UnitNotUsedFunction";
import { markManyUnitsUsed } from "../../unit/function/UnitUsedFunction";

/** 화면 표시에만 쓰는 행 타입 */
type Row = {
  id: number;      // PK
  unit_id: string; // 화면용 ID (UNIT_0001)
  code: string;
  name: string;
};
...
const NotUsedUnitPageUi: React.FC = () => {
  ...
  const to_row = (u: ApiUnit): Row => ({
    id: Number((u as any).id),
    unit_id: String((u as any).unit_id ?? "").trim(),
    code: String((u as any).code ?? "").trim(),
  ...
  const load = async () => {
    ...
    set_error("");
    try {
      const list = await fetchNotUsedUnits();
      set_rows((Array.isArray(list) ? list : []).map(to_row));
      set_checked({});
    } catch (e: any) {
      set_error(e?.message || "미사용 단위 목록을 불러오지 못했습니다.");
    } finally {
      set_loading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    window.addEventListener("unit:notused:updated", onNotUsed);
    window.addEventListener("unit:used:restored", onRestored);
    return () => {
      window.removeEventListener("unit:notused:updated", onNotUsed);
      window.removeEventListener("unit:used:restored", onRestored);
    };
  }, []);
...
export default NotUsedUnitPageUi;
